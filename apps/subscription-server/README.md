...
Node.js 服务从最后有效缓存分发 FlyingBird、LeapVPN 和 MonoCloud 订阅。每家使用独立读取 token；Nginx 保护管理入口，Node 后台仅监听回环地址。服务复用 `@proxy-toolkit/flybird` 和 `@proxy-toolkit/monocloud` workspace 包；飞跃自动登录和导出由 `providers/leapvpn/` 中的 Python 包完成。
...
/opt/private-subscription/                        ← 完整仓库
  ...
  providers/monocloud/                             ← MonoCloud 包源码
...
3. 在私有终端保存初始化结果。脚本创建缓存、token 和状态目录，生成三家读取 token 及管理密码；这些输出不要进入 CI、日志收集或公开文档。
...
systemctl start private-subscription-refresh@monocloud.service
...
FlyBird 每次刷新都会重新读取上游入口；新入口遇到 DNS 校验失败时短暂重试并再次拉取，最多 3 次。入口优先使用服务器系统 DNS 验证；系统 DNS 无记录时，仅当订阅同时声明至少两个可解析的 HTTPS `proxy-server-nameserver`，才保留域名交给 Mihomo 客户端的节点专用解析链，并在缓存元数据记录 `delegatedHostCount`。缺少这组条件仍会拒绝发布并保留最后有效缓存。
...
依赖统一在仓库根目录安装。从应用目录可直接执行 `npm test` 与 `npm run check`。刷新测试使用真实共享分流函数、合成上游与临时目录，覆盖缓存、token 隔离和失败路径；不会访问真实 provider。
