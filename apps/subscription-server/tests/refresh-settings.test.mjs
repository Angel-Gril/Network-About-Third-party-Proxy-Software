...
test("refresh settings default to six hours and persist provider overrides", async () => {
  ...
  try {
    ...
    assert.deepEqual(defaults.providers.monocloud,{enabled:true,intervalMinutes:360});
    await store.update("monocloud",{enabled:false,intervalMinutes:720});
    const saved = await store.get();
    assert.deepEqual(saved.providers.monocloud,{enabled:false,intervalMinutes:720});
    assert.deepEqual(saved.providers.flybird,{enabled:true,intervalMinutes:360});
...
test("refresh timing uses the latest success or failure and honors the switch", () => {
  ...
  assert.equal(refreshTiming(metadata,{enabled:true,intervalMinutes:180},now).due,false);
  assert.deepEqual(refreshTiming(metadata,{enabled:false,intervalMinutes:60},now),{
    enabled:false,intervalMinutes:60,due:false,nextAt:null,
  });
});
