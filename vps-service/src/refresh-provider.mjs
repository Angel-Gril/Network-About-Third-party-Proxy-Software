...
async function refreshFlyBird() {
  ...
  try {
    const modulePath = pathToFileURL(process.env.FLYBIRD_WORKER_MODULE).href;
    const { handleRequest } = await import(modulePath);
...
async function refreshLeapVpn() {
  ...
  if (!python || !exporter || (automaticLogin ? !credentials || !authState : !session)) {
  ...
  try {
    ...
    try {
      const args = automaticLogin
        ? [exporter, "--credentials", credentials, "--state", authState, "--out-dir", output]
        : [exporter, "--settings", session, "--fetch-all", "--out-dir", output];
    ...
    const workerModule = process.env.FLYBIRD_WORKER_MODULE;
    const domain = process.env.PUBLIC_DOMAIN;
    if (!workerModule || !domain) throw new Error("Shared routing configuration is incomplete");
    const { enhanceMihomoConfig } = await import(pathToFileURL(workerModule).href);
