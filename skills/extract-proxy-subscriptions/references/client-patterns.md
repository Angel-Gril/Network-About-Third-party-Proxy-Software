...
Windows `3.1.8+2026071612` 已改变这一链路：订阅请求使用 `securitynet/v3.1.8 clash-verge Platform/windows`，旧标识会在有效订阅 token 下收到 `401 invalid token`。响应为 Base64 包装的 `12 字节 nonce | AES-256-GCM 密文 | 16 字节 tag`，密钥由客户端内置配置常量的 SHA-256 得到。首选项采用相同 GCM 布局及 `enc1:` 前缀，客户端磁盘 profile 另有循环异或包装。必须分别验证网络响应和本地文件，不能仅凭修改时间推断在线刷新成功。

当前实现对 GCM 验证认证标签，并保留旧 CBC 兼容；地址首选项解码只用于发现 API，不读取保存的账号密码。Windows PowerShell 5.1 的 GCM 由系统 CNG 提供，其他受支持 PowerShell 运行时使用 .NET。认证失败不输出解密片段，不覆盖最后有效文件。
