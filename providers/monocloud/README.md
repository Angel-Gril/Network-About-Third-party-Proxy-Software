...
输出目录必须不存在。生成 `monocloud_clash.yaml`、`monocloud_ss.txt` 和脱敏的 `monocloud_meta.json`。前两个文件包含真实连接凭据，已由根 `.gitignore` 排除。
...
客户端还会请求 `/api/bandwidth/<id>` 显示流量，但导出连接参数不依赖该接口。认证请求需要客户端版本头；账号 token 与 VPS 的订阅读取 token 是两类不同凭据。
...
合成测试覆盖认证字段、备用 API、限流停止、错误脱敏、套餐边界、节点转换和 CLI 私有输出。真实验证另行报告 DNS、选定出口转发与规则命中；一个出口成功不代表全部节点可用。
