...
test("provider source does not reference Node-only Buffer globals", () => {
  const source = readFileSync(new URL("../src/subscription.js", import.meta.url), "utf8");

  assert.equal(/\bBuffer\b/.test(source), false);
});

test("launcher always uses the local account PowerShell exporter", () => {
  const source = readFileSync(new URL("../export.bat", import.meta.url), "utf8");

  assert.match(source, /export\.ps1/);
  assert.doesNotMatch(source, /export\.py/);
...
test("PowerShell exporter logs in, downloads, and decrypts on the local machine", () => {
  const source = readFileSync(new URL("../src/export.ps1", import.meta.url), "utf8");
...
test("PowerShell exporter rejects decryptable subscriptions with no resolvable proxy hosts", () => {
  const exporter = readFileSync(new URL("../src/export.ps1", import.meta.url), "utf8");
  const validation = readFileSync(new URL("../src/validation.ps1", import.meta.url), "utf8");
...
test("routing template uses compact MetaCubeX MRS providers", () => {
  const source = readFileSync(new URL("../templates/mihomo.yaml", import.meta.url), "utf8");
...
test("Python compatibility entry delegates account arguments to PowerShell", () => {
  const source = readFileSync(new URL("../src/export.py", import.meta.url), "utf8");
