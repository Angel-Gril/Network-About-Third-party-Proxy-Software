...
def receiver_module():
    spec = importlib.util.spec_from_file_location('receiver_subject', Path(leapvpn.__file__).with_name('receive.py'))
...
class PortableSessionToolsTests(unittest.TestCase):
    def test_installed_module_entries_work_outside_the_checkout(self):
        with tempfile.TemporaryDirectory() as directory:
            for module in ("leapvpn.export", "leapvpn.refresh", "leapvpn.sync"):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for extra in ([], ['--host', 'operator@host.example.invalid']):
                with self.subTest(arguments=extra):
                    result = subprocess.run([sys.executable, '-B', '-m', 'leapvpn.sync',
    ...
    def test_failed_first_sync_leaves_new_state_owned_by_the_service_account(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(subject.pwd, 'getpwnam', return_value=account, create=True), \
                 ...
                 patch('leapvpn.auth.import_client_session', side_effect=ExportError('Synthetic import failure')):
